...
type Props = {
    ...
    value?: string | number | dataObj | null
...
export default function Dropdown(props: Props) {
    ...
    const renderSelectedItem = () => {
        return <div
            ...
            style={{
                border: color ? `1px solid ${color}` : openDrop ? '1px solid #bb764c' : '1px solid lightgray',
            ...
            <h4 className={`dropdown__selected`} style={{ color: color || '', backgroundColor: bgColor || '' }}>
                {getSelectValue()}
            </h4>
            < h4 className={`dropdown__selected`} style={{ color: color || '', backgroundColor: bgColor || '' }}>▾</h4>
        </div>
    }
    ...
    const renderSelectedItems = () => {
        return <div
            className={`dropdown__select`}
            style={{
                border: openDrop ? '1px solid #bb764c' : '1px solid lightgray',
                borderBottomRightRadius: openDrop ? 0 : '',
                borderBottomLeftRadius: openDrop ? 0 : '',
                filter: openDrop ? 'brightness(95%)' : ''
            }}
            ...
            {options?.length ?
                options.map((option: any, i: number) =>
                    <h4
                        ...
                        {isDate ? new Date(option).toLocaleDateString(locale || 'es-ES') :
    ...
    const renderMultiSelect = () => {
        return (
            <div className={`dropdown__container`} style={{ ...style, backgroundColor: bgColor || '' }}>
